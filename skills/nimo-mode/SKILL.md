...
进入已授权开发时，任务持有者用 [task-anchor.mjs](scripts/task-anchor.mjs) 初始化或修订 `.nimo/tasks/<task-id>/task.json`。合同修订保存确认来源和完整验收快照；验证记录只引用 `contractRevision`，不能降低任务记录中的 `required`。
...
知识请求直接路由，不伪装成普通 Playbook：

- 用户明确要求“审计 / 检查知识是否过时、漂移、索引是否正确”时使用 [nimo-knowledge-audit](../nimo-knowledge-audit/SKILL.md)。第一版只接受用户显式请求或定时/外部例程明确调用，普通工程任务不得主动触发。
- 用户明确要求“建立 / 刷新 / 同步 / 重建项目知识”时使用 [nimo-knowledge-maintain](../nimo-knowledge-maintain/SKILL.md)。它是写操作，必须有当前请求或预先配置例程的明确写入授权。

...
5. 按 [nimo-verify](../../nimo-verify/SKILL.md) 在匹配的操作面上验证。这是实现阶段验证，用于快速确认实现方向正确；它不是最终交付证据——最终交付证据是所有 cleanup／review 修复／提交组织完成之后的 Final Verify。"无法判定"或操作面错误不算通过；标记出来。
...
8. 本次变更改变了已登记在 `.nimo/verification/` 中的用户入口、关键操作步骤、稳定定位方式、CLI 命令、路由、选择器、可观察副作用、关键用户状态或预期用户行为时，更新受影响 Verification Map 条目或辅助脚本，并用更新后的路径执行验证。项目没有对应 map 时不阻塞普通修改，但不得声称存在对应验证覆盖。不要求每次执行完整 [nimo-verification-maintain](../../nimo-verification-maintain/SKILL.md) 巡检，只维护受本次变更影响的条目。
9. 按授权运行 [整理并创建 PR](opening-a-pr.md)。Final Verify 由 opening-a-pr 承担；没有推送／PR 授权时，仍在所有 cleanup／review／map 更新完成后完成本地 Final Verify，再停在本地交付结果——PR 授权与验证职责不绑死。
...
必须先明确领域数据形状及其组织结构，再让委派者写逻辑。`nimo-architect` 跳过时保留原因；跨边界或存在实质设计取舍时不能用"任务小"自动跳过；architect 只交付设计，实现由委派者承担。吞吐量检查点四项必须保留，不适用的标 `n/a` 和原因，不许静默折叠成实现细节；实现存在多种有效形状时必须走 nimo-arena，没有 skip-with-reason 逃逸，nimo-principle-laziness-protocol 不覆盖它。主 Agent 亲自审每个委派者的 diff；"无法判定"或操作面错误的验证不算通过；实现与最终审查分开；子 Agent 各自通过后仍须验证整合产物。实现阶段验证不是最终交付证据，最终交付证据是全部 cleanup／review 修复／提交组织之后的 Final Verify；interrogate 的"处理"项阻塞交付，修复后重新验证；已登记验证路径受本次变更影响时同步更新地图再验证。频繁提交限于本地；推送、PR、评论、合并等外部动作受用户当前授权约束。

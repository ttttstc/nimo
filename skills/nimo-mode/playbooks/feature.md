...
进入本流程时，Mode 已要求为实际变更任务初始化 Task Audit。继续前确认 `<projectRoot>/.nimo/tasks/<task-id>/audit.md` 已存在且 Contract 中包含本次全部验收场景；缺失时先按 [nimo-show-me-your-work](../../nimo-show-me-your-work/SKILL.md) 的协议补齐初始化，不进入设计或实现。后续实质选择及时追加，不把 Decision 留到任务结束统一回忆。

1. 对受影响子系统运行 [nimo-how](../../nimo-how/SKILL.md)。把本次实际使用记录到 Task Audit 的 Harness 区；只读取 Skill 不等于已应用，Evidence of use 指向本步骤产物或后续 Decision。
2. 用 [nimo-architect](../../nimo-architect/SKILL.md) 做并行设计探索。architect 只交付设计（候选、综合决策与类型／签名草案），实现由第 4 步承担。跳过时保留记录 `nimo-architect skipped: <原因>`；不许把设计决定静默折叠进实现。方案收口会实质改变 Artifact、Scope、Risk、Acceptance 或 Verification 时，立即按 `nimo-show-me-your-work` 追加 design Decision 与可检查 Evidence。
...
交付时说明：构建了什么、做了哪些选择及为什么、每个验收场景如何验证、Task Audit 位置、未决事项。设计备选方案用表格呈现。
...
返回结果、当前产物版本、Task Audit 位置、验收场景到验证资产／证据的映射、未完成项及跳过原因。流程不自动授予提交、推送、评论、合并或发布权限。

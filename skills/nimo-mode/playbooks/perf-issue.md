...
用户明确要求修复性能问题即可进入，不必绕经 Create 再次确认。若从测试建设发现问题，先按 [Create 的路由规则](../../nimo-verification-create/SKILL.md#0-确定建设范围与性能授权) 确认产品优化授权；仅补测试的请求停在测试资产和发现报告。

1. 读取 [性能用例与测量协议](../../nimo-verification-create/references/performance-testing.md)，固定用户操作、真实可用终点、负载、指标与正确性护栏。缺少可复用资产时调用 [nimo-verification-create](../../nimo-verification-create/SKILL.md) 只补资产再返回。由 [nimo-verify](../../nimo-verify/SKILL.md) 在匹配的操作面上采集基线 trace 和护栏结果；没有有效基线不开始宣称可比较的优化。
...
3. 从 trace 规划修复。跨函数边界时先跑 [nimo-architect](../../nimo-architect/SKILL.md)。把实现委派给子 Agent（用户配置的性能问题模型）；亲自审阅它的 diff。宿主没有独立子 Agent 时，主 Agent 顺序扮演实现者并记录降级，降级下的自审不称独立审查。由 `nimo-verify` 按冻结协议采集修复后 trace 并运行正确性护栏。
   运用 [nimo-principle-sequence-verifiable-units](../../nimo-principle-sequence-verifiable-units/SKILL.md)：每次尝试验证通过后，再试下一个。
4. 解析并比较产物（JSON 转 sqlite、diff）。最终耗时结论使用同条件、同一时段的 A/B 配对测量，交替轮换先后顺序。按共享协议报告失败样本、噪声和适用范围；"无法判定"或操作面错误不算通过。确认收益后，由 Create／Maintain 固化回归用例和经校准的阈值，再交给 `nimo-verify` 验证；不自动接 CI 或修改已确认的体验预期。
5. 在 PR 里引用测量。

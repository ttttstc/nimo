...
用户明确要求持续优化即可进入；仅有性能测试授权时，不自动启动本循环，按 [Create 的路由规则](../../nimo-verification-create/SKILL.md#0-确定建设范围与性能授权) 确认。性能目标按需读取 [性能用例与测量协议](../../nimo-verification-create/references/performance-testing.md)；非性能指标不套用耗时协议。目标达到后按约定停止，不因发现新指标而自动扩展任务。

1. 先给工作负载和架构打地基，再选尺子。对目标运行 [nimo-how](../../nimo-how/SKILL.md)，点名能移动结果的现实工作负载维度（数据规模、历史、状态、并发），选择一个能复现用户抱怨的场景。没有场景能复现它，先修复现，不爬坡。然后固定指标、改善方向、目标、预算与停止条件。用户已给出时直接使用，只澄清缺失且会改变结论的条件。最低尝试次数仅在用户已约定时适用；用重复测量排除偶然改善，不用额外修改轮数代替测量充分性。
2. 构建测量 harness，证明它的灵敏度，然后冻结它（[nimo-principle-build-the-lever](../../nimo-principle-build-the-lever/SKILL.md)）。跑对比性的现实工作负载，确认目标场景复现症状、更容易的场景按预期分开。尺子区分不了它们，就改工作负载或指标。一旦冻结，一条可重复的命令发出指标，按场景固定统计口径并重复采样，不用单次运行作结论；改动口径后，旧数据不能直接用于新对比。性能资产缺失时由 Create 补齐后返回本流程；代理指标先证明与真实结果的关联，再用于优化。任何改动之前，由 [nimo-verify](../../nimo-verify/SKILL.md) 记录基线指标和回归门禁（必须保持通过的测试）的一次绿色运行。
...
5. 循环，每轮一个假设：
   ...
   - 由 `nimo-verify` 用冻结的 harness 测量前后，并跑回归门禁。性能测量的失败样本不能丢弃后当作通过；测量时避免多个构建或测速任务争抢同一机器资源。
...
8. 性能目标交付前，由 `nimo-verify` 做同条件、交替顺序的 A/B 配对复测。将确认收益交给 Create／Maintain 固化回归资产；阈值检查只读，收紧阈值与日常检查分开。按授权运行 [整理并创建 PR](opening-a-pr.md)，被接受的 commit 按落地顺序堆叠，让指标的爬升从上到下可读。

交付时说明：指标与目标、基线到最终的百分比差值、运行轮数（保留 vs 撤销）、每个被接受的修复一行、决策日志路径，以及如果继续推进，下一个最想试的想法。

## 必要条件与停止

平台期应更换假设，不降低目标。达到经复测确认的目标及用户已约定的尝试要求、耗尽预算、用户停止或真实无可行路径时结束，并区分"目标达到"与"未达到后停止"。预算耗尽或用户停止优先于最低轮数，不自行编造目标或轮数。目标未达到且预算允许时继续探索；卡住浮出，不空转。

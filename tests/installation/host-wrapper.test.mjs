...
async function assertLifecycle(host, root, install, uninstall, homeArgs) {
  ...
  assert.equal((await skillDirectories(skillsRoot)).length, 46, `${host.name} wrapper installed all 46 Skills`);
...
test('Bash wrappers pass syntax checks and smoke install/uninstall when Git Bash is available', async t => {
  ...
  for (const host of HOSTS) {
    ...
    try {
      ...
      const skillsRoot = join(home, 'skills');
      assert.equal((await skillDirectories(skillsRoot)).length, 46, `${host.name} Bash wrapper installed all 46 Skills`);

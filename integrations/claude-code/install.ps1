# nimo Claude Code 接入：安装/更新脚本（Windows PowerShell）
# 用法:
#   .\install.ps1                               安装到默认 CLAUDE_CONFIG_DIR（或 ~/.claude）
#   .\install.ps1 -ClaudeConfigDir <dir>        安装到隔离目录（测试用）
#   .\install.ps1 -Source <repo-root>           指定 nimo 仓库位置（默认取脚本上级目录）
# 行为:
#   - 只写入 <ClaudeConfigDir>\skills 下 nimo 自有文件，并记录清单（.nimo-manifest）

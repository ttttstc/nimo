...
大家把越来越多的活交给 agent：写得越来越快，验证得越来越少，交付越来越像抽奖。质量是交付吞吐量的前提，想走得快，先挖得深，建好护栏。

**nimo 是我想做的护栏。** 它不运行模型、不执行工具——那是宿主（Codex、Claude Code、OpenCode、Cursor、dsh）的事。nimo 做的是工程纪律本身：原则、任务做法、可替换的研发能力、真实验证、持续改进。目标不是最大化代码行数，恰恰相反：**nimo 帮你写得更少，测得更稳，结果都有证据。**
...
完整定义、适用边界和三层控制模型见 [Nimo 设计原则](docs/principle.md)。
...
```text
"使用 nimo skill：看看这个项目接下来应该怎么推进，先讨论，不要修改任何文件。"
```
...
| **工程方法层**  | **nimo**                                      | 原则、playbook、能力合同、验收规则、知识/验证维护、评测维护 |
| 项目资产层      | 你的项目与已登记知识路径                                 | 代码、原位知识、功能地图、验证脚本、检查点               |
| 强制控制层      | 你的 CI、仓库保护与发布控制                               | 合并、发布等结果门禁的强制执行                    |

nimo 定义「怎样才算做对了、做完了」以及 Gate 所需的条件与证据；宿主管执行能力边界，具备控制权的 CI、仓库保护等系统负责不可绕过的强制门禁。

## 工程原则

这里的 23 条原则是具体任务执行时使用的工程纪律；更高一层、约束 Nimo 自身架构与演进方式的七大设计原则见 [Nimo 设计原则](docs/principle.md)。入口在任务开始时读工程原则索引，按需展开，不逐条罗列原则名。
...
- 不替代你的 CI 和仓库保护。nimo 可以定义验收条件和证据要求，但不可绕过的强制门禁必须由具备控制权的系统执行；提示词也不是安全沙箱。
...
- [Nimo 设计原则](docs/principle.md)：七大设计原则、适用边界以及 Harness / Runtime / Gate 控制模型。
